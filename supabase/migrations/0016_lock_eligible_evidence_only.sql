...
begin
  ...
  join public.docket_items docket
    on docket.id = source.docket_item_id
  join public.cases case_record
    on case_record.id = source.case_id
  where evidence.case_id = target_case_id
    and case_record.id = target_case_id
  ...
  if evidence_projection is null or jsonb_array_length(evidence_projection) = 0 then
    raise exception 'active eligible evidence is required' using errcode = 'P0001';
  end if;
